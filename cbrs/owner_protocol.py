        ...
        while time.monotonic() < deadline:
            ...
            if row["state"] in {"failed", "uncertain", "cancelled"}:
                ...
                if isinstance(error, dict) and error.get("reason") in {reason.value for reason in StopReason}:
                    stop = SafetyStopException(StopReason(error["reason"]), "Browser owner reported a portal failure", status=error.get("status"), context=error.get("context") or "browser owner", response_code=error.get("response_code"))
                ...
                if isinstance(error, dict) and error.get("reason") == "credentials_invalid":
                    raise CredentialsRejectedError(status=error.get("status"), response_code=error.get("response_code"))
