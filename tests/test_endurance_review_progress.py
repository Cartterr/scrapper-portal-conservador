...
def test_unknown_job_ends_without_replay_and_next_fixture_can_proceed(tmp_path):
    ...
    assert finish_for_review(store,first['job_id'],'search_outcome_unknown')
    old=store.get_job(first['job_id'])
    assert old['status']=='failed' and old['error_code']=='search_outcome_unknown'
    assert controller.maybe_enqueue() is None  # Retains normal pacing.
    second=controller.maybe_enqueue(force=True)
    assert second and second['job_id']!=first['job_id']
    assert store.claim_next('worker').job_id==second['job_id']
