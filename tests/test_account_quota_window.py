...
def test_fixed_windows_survive_midnight_restart_and_expire_exactly(tmp_path):
    ...
    with sqlite3.connect(path) as db:
        db.executescript('CREATE TABLE jobs(job_id TEXT,source TEXT); CREATE TABLE job_attempts(job_id TEXT,account_id TEXT,quota_consumed INTEGER,status TEXT,finished_at TEXT,started_at TEXT);')
        for i,(account,offset,status) in enumerate([('a',0,'search_completed'),('a',4,'search_completed'),('a',5,'failed'),('b',6,'search_completed')]):
            ...
            db.execute('INSERT INTO job_attempts VALUES(?,?,?,?,?,?)',(str(i),account,1,status,stamp,stamp))
