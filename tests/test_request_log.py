...
def test_records_only_portal_calls_without_query_or_tokens(tmp_path):
    ...
    request_log.record(directory, _request(f"https://nuevo-portal.conservador.cl/api/v1/doc/{ticket}/img?page=2&t=x",
                                           method="POST"), 200, now=at)
    request_log.record(directory, _request("https://www.google.com/recaptcha/api2/reload"), 200, now=at)
    ...
    assert [json.loads(line) for line in lines] == [{
        ...
        "path": "/api/v1/doc/:id/img", "type": "xhr", "status": 200}]
...
def test_window_read_and_per_endpoint_summary(tmp_path):
    ...
    directory = request_log.request_log_dir(settings)
...
def test_real_chrome_context_records_responses_and_failures(tmp_path):
    ...
    with sync_api.sync_playwright() as playwright:
        ...
        request_log.attach(context, settings)
