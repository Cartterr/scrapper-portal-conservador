...
$snapshotPath = Join-Path 'C:\ProgramData\CBRS' ('session-snapshot-' + [guid]::NewGuid().ToString('N'))
...
$sources = @{
    'cbrs.env' = 'C:\ProgramData\CBRS\cbrs.env'
    'account-pool.json' = 'G:\CBRS\account-pool.json'
    'endurance-plan.json' = 'G:\CBRS\endurance-plan.json'
}
foreach ($index in 1..3) {
    $sources["ejecutivo_$index-baseline.json"] = "G:\CBRS\accounts\ejecutivo_$index\fixed-egress-baseline.json"
}
